...
describe("getSortedPostsFrontMatter returns front matter filtered and sorted", () => {
  ...
  it('should return all published posts', () => {
    ...
    expect(postTitles).not.toContain("Hello World 3"); // TODO this doesn't test anything, try changing the test to random words
  });
  ...
  it('Should return title', () => {
    ...
    expect(title).toBe("Hello World 1");
  });

  it('Should not return post if title is missing', () => {
    const posts = getSortedPostsFrontMatter();
    const filteredPosts = posts.filter((post) => {
      return post.excerpt === "Missing title excerpt"; // TODO this doesn't test anything either. 
    });
  ...
  it('Should return excerpt', () => {
    ...
    expect(excerpt).toBe("Hello World 2 excerpt");
  });

  it('Should not return post if excerpt is missing', () => {
    const posts = getSortedPostsFrontMatter();
    const filteredPosts = posts.filter((post) => {
      return post.title === "Missing excerpt wank";
    });
    expect(filteredPosts).toHaveLength(0);
  });

  // TODO Hide posts that do not have the other mandatory information
});
